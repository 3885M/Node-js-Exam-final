...
const { recipe_model, loginModel } = require("../model/model");
...
const home = async (req, res) => {
    try {
        const show = await recipe_model.find();
        console.log(show);
...
const addrecipe = async (req, res) => {
    try {
        ...
        const addDatarecipe = await recipe_model.create({
...
const deleterecipe = async (req, res) => {
    let id = req.query.id;
    try {
        let Data = await recipe_model.findById(id);
        console.log("delete data" + Data);
        let imgpath = Data.image;
        let deleteData = await recipe_model.findByIdAndDelete(id);
...
const editPage = async (req, res) => {
    try {
        let data = await recipe_model.findById(req.query.id);
...
const update = async (req, res) => {
    try {
        ...
        let updateData = await recipe_model.findByIdAndUpdate(
            id,
            {
                title: req.body.title,
                author: req.body.author,
